...
var _excluded = ["bitmap", "indices", "offsets", "options"],
  _excluded2 = ["color", "innerColor", "interpolationFactor"];
import { useFrame } from "@react-three/fiber";
import React, { useEffect, useRef, useState } from "react";
import { AdditiveBlending, CanvasTexture, Float32BufferAttribute, MathUtils, Uint16BufferAttribute, Vector2, Vector3 } from "three";
...
var Particles = function Particles(_ref) {
  ...
  useFrame(function (_ref2) {
    ...
    for (var key in rest) {
      var name = "u" + (key.charAt(0).toUpperCase() + key.slice(1));
      particlesMaterialRef.current.uniforms[name].value = MathUtils.lerp(particlesMaterialRef.current.uniforms[name].value, options[key], options.interpolationFactor);
    }
  });
  useEffect(function () {
